(function(window1) {
    "use strict";
    var docElem = window1.document.documentElement, transEndEventNames = {
	 ...
	 */ function getViewport(axis) {
        ...
        if (axis === "x") {
            ...
            inner = window1["innerWidth"];
        } else if (axis === "y") {
            client = docElem["clientHeight"];
            inner = window1["innerHeight"];
        }
	 ...
	 */ DragSlideshow.prototype._initEvents = function() {
        ...
        this.slides.forEach(function(slide) {
            ...
            slide.addEventListener("click", function() {
                ...
                if (self.slides.indexOf(slide) === self.current) self.toggle();
                else self.dd.setStep(self.slides.indexOf(slide) + 1);
            });
            // reveal content
        ...
        document.addEventListener("keydown", function(ev) {
            var keyCode = ev.keyCode || ev.which, currentSlide = self.slides[self.current];
            if (self.isContent) switch(keyCode){
                // up key
                case 38:
                    // only if current scroll is 0:
                    if (self._getContentPage(currentSlide).scrollTop === 0) self._toggleContent(currentSlide);
                    break;
            }
            else switch(keyCode){
                // down key
	 ...
	 */ DragSlideshow.prototype._toggleContent = function(slide) {
        ...
        var self = this, onEndTransitionFn = function(ev) {
            ...
            if (self.isContent) classie.remove(page, "show");
            self.isContent = !self.isContent;
            self.isAnimating = false;
            // callback
            self.options.onToggleContentComplete();
        };
        if (support.transitions) this.el.addEventListener(transEndEventName, onEndTransitionFn);
	 ...
	 */ DragSlideshow.prototype.toggle = function() {
        ...
        var self = this, p = this.options.perspective, r = this.options.slideshowRatio, zAxisVal = this.isFullscreen ? p - p / r : p - p * r;
        ...
        var onEndTransitionFn = function(ev) {
            ...
            if (!self.isFullscreen) // remove preserve-3d to the slides (seems to fix a rendering problem in firefox)
        ...
        if (support.transitions) this.imgDragger.addEventListener(transEndEventName, onEndTransitionFn);
        else onEndTransitionFn();
    };
    /**
	 ...
	 */ window1.DragSlideshow = DragSlideshow;
})(window);

//# sourceMappingURL=index.33d34dd7.js.map
